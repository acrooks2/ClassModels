globals [cities conflict halfedge edge north-camp-counter east-camp-counter south-camp-counter west-camp-counter idp-camp-counter nearby]; [] ;; keep track of how many cities and conflict there is
;; rebels, government, and refugees are all breeds of turtle.
breed [rebels rebel]  ;; comment
breed [government government-unit]
breed [refugees refugee]
breed [camps camp]
refugees-own [evacuation]
...
to setup
  ...
  create-government initial-number-government  ;; create the government, then initialize their variables
  [
    randomize
    set color red
    set size 1  ;; easier to see
...
to randomize
  setxy random-xcor random-ycor
  if pcolor = brown or pcolor = blue or pcolor = green or pcolor = pink or pcolor = orange or pcolor = magenta     ; if it's on the boundary or other country
    [ randomize ]        ; ...try again
end
...
to set-label
  ...
  ask patches [
    ...
    if (pxcor = 32 and pycor = 0)
    [set east-camp-counter count turtles-here]
    if (pxcor = 0 and pycor = -32)
    [set south-camp-counter count turtles-here]
    if (pxcor = -32 and pycor = 0)
...
to move  ;; turtle procedure
  ;; make the agents not cross over the border to other countries
  ifelse [pcolor] of patch-ahead 3 = blue or [pcolor] of patch-ahead 2 = brown or [pcolor] of patch-ahead 2 = green or [pcolor] of patch-ahead 2 = orange or [pcolor] of patch-ahead 2 = pink
  [
  rt random 180
  fd 1
  ]
  ;; right random 360  Try to add random 360 for 1st move
  [
  rt random 50
  lt random 50
  fd 1
...
to evacuate ;; refugee procedure
    ;;makes sure the refugees stop fidgeting around if they are at a camp
    if (evacuation = 1 and pcolor != magenta) [
      if (pcolor != violet) [
        if (pcolor != grey) [
          face min-one-of patches with [ pcolor = magenta or pcolor = violet] [ distance myself ]
          forward 1
        ]
      ]
    ]
...
to end-conflict  ;; patch procedure
  ;; countdown on red patches: if reach 0, end conflict
  if pcolor = red [
    ifelse countdown <= 0
      [ set pcolor black
        set countdown conflict-end-time ]
...
to reach-camp-limit
  ...
  [
    if pcolor = violet [
      if count turtles-here > IDP-camp-limit [
...
"undamaged city area" 1.0 0 -8053223 true "" "plot count patches with [pcolor = yellow]"
